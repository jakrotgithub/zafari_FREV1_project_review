...
server <- (function(input, output, session) {
  ...
  #addTooltip(session,'int_effect', 'If the patient is going to use a new intervention, please indicate the effect of the new intervention relative to his/her current therapy on initial improvement in lung function (L). If you only want to model the natural course of disease progression irrespective of specific intervention, please select 0 in here.'))
	...
	output$figure<-renderPlotly({
		...
		if (!is.null(input$fev1_0) & input$model=="Basic model with only baseline FEV1") {
		  ...
		  df <- fev1_projection(input$fev1_0, input$int_effect, graph=T)$df
	...
	output$cv<-renderTable({
      ...
      aa4 <- fev1_projection4(input$fev1_0, input$fev1_prev, input$int_effect, sex=input$sex, smoking=input$smoking, input$age, input$weight,
                              input$height, input$oco)$aa4
